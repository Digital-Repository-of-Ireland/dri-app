...
THIS 1.0 release is a snapshot
...
If you are using homenbrew on OSX
  ...
  $ brew install ansible coreutils libtool libksba libxml2 libxslt automake autoconf openssl curl curl-ca-bundle readline lzlib gettext pkg-config libyaml mariadb redis clamav sqlite ffmpeg gource phantomjs fits glib imagemagick node
...
To view your queues (for debugging purposes) run
  ...
  $ resque-web -x -N dri:development

This should pop up a browser window looking at localhost:5678 (if not open one). Here
you can see the status of your queues and jobs. This is also mounted in the application
at /resque
