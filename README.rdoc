...
If you are using Ubuntu or another debian linux distribution
  ...
  $ sudo apt-get install coreutils libtool libksba8 libxml2 libxslt1.1 automake autoconf openssl curl libreadline8 gettext pkg-config libyaml-dev redis sqlite ffmpeg libgtk2.0-dev imagemagick ghostscript npm
...
With the above installed the Javascript can be compiled with: bin/rails javascript:build. Follow the jsbundling-rails gem README for more instructions.
...
you will need access to an S3 server. MiniStack (or another similar S3 compatible emulator) can be used 
