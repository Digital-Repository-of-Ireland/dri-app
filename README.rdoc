...
An alternative, if the Ceph VM is not working for you, is to use the FakeS3 gem. Install
 the gem with:
...
Create a directory for the server to use to store files, and then start the server with:
 ...
 $ sudo fakes3 -r /path/to/fakes3_root -p 80 -h localhost
...
To use a port other than port 80, change the server setting to be:

  server: localhost:8081/

and start fakes3 using this port number

 $ fakes3 -r /path/to/fakes3_root -p 8081
