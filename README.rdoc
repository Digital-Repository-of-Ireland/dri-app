...
An alternative, if the Ceph VM is not working for you, is to use LocalStack. This can be installed
using pip:
...
In development, if you would like the files to persist LocalStack restarts, create a directory for the server
to use to store files, and then start the server with:

 $ SERVICES=s3:8081 DATA_DIR=/path/to/localstack_files localstack start
