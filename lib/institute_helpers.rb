module InstituteHelpers
  ...
  def self.get_object_depositing_institute_from_solr_doc(doc)
    unless doc[Solrizer.solr_name('isGovernedBy', :stored_searchable, type: :symbol)].nil?
      id = doc[Solrizer.solr_name('isGovernedBy', :stored_searchable, type: :symbol)][0].gsub(/^info:fedora\//, '')
      solr_query = "id:#{id}"
