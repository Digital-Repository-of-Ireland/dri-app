module InstituteHelpers
  ...
  def self.get_depositing_institute_from_solr_doc(doc)
    doc[ActiveFedora::SolrQueryBuilder.solr_name('type', :stored_searchable, type: :string)].include?('Collection') ? get_collection_depositing_institute_from_solr_doc(doc) : self.get_object_depositing_institute_from_solr_doc(doc)
  end
  ...
  def self.get_object_depositing_institute_from_solr_doc(doc)
    ...
    if collection[ActiveFedora::SolrQueryBuilder.solr_name('depositing_institute', :displayable, type: :string)].present?
        Institute.where(:name => collection[ActiveFedora::SolrQueryBuilder.solr_name('depositing_institute', :displayable, type: :string)]).first
    else
      nil
    end
  end
