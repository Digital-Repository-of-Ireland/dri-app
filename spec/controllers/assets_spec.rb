...
describe AssetsController do
  ...
  describe 'create' do
    ...
    it 'should create an asset from a local file' do
      ...
      FileUtils.cp(File.join(fixture_paths.first, "SAMPLEA.mp3"), File.join(tmp_upload_dir, "SAMPLEA.mp3"))
    ...
    it 'should create an asset from an upload' do
      ...
      uploaded = Rack::Test::UploadedFile.new(File.join(fixture_paths.first, "SAMPLEA.mp3"), "audio/mp3")
   ...
   describe 'update' do
    ...
    it 'should create a new version from a local file' do
      ...
      FileUtils.cp(File.join(fixture_paths, "SAMPLEA.mp3"), File.join(tmp_upload_dir, "SAMPLEA.mp3"))
    ...
    it 'should not remove the file if save fails when replacing with matching file but different filenames' do
      ...
      FileUtils.cp(File.join(fixture_paths, "SAMPLEA.mp3"), File.join(tmp_upload_dir, "SAMPLEA_copy.mp3"))
  ...
  describe 'download' do
    ...
    it "should be possible to download the master asset" do
      ...
      expect(response.header['Content-Length']).to eq("#{File.size(File.join(fixture_paths, "SAMPLEA.mp3"))}")
    end
  end
