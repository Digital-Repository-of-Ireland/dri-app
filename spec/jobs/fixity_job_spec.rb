...
describe 'FixityJob' do
  ...
  describe 'run' do
    it "should set create a FixityCheck for a collection\'s objects" do
      ...
      expect{ FixityJob.perform(report.id, @collection.alternate_id, @collection.alternate_id) }.to change(FixityCheck, :count).by(1)
      expect(FixityCheck.find_by(object_id: @object.alternate_id).verified).to be true
    end
  end
end
