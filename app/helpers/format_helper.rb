module FormatHelper
  ...
  def format?(document)
    if document[Solrizer.solr_name('file_type', :stored_searchable, type: :string)].present?
      document[Solrizer.solr_name('file_type', :stored_searchable, type: :string)].first
    else
      'unknown'
    end
  end
